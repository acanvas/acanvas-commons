...
class Flow extends BoxSprite with MFlow {
  Flow(
      /*{FlowOrientation flow : FlowOrientation.HORIZONTAL, double spacing: 0.0, bool distribute: false, bool snapToPixels: true, bool inverted: false, bool animate: false, bool reflow: false, AlignV alignV : AlignV.TOP}*/) {
    //inheritSpan = true;
    autoRefresh = true;
  }
  ...
  void refresh() {
    ...
    children.forEach((child) {
      ...
      _prevChildWidth = _prevChild == null
          ? 0
          : _prevChild is MBox
              ? (_prevChild as MBox).spanWidth
              : _prevChild.width;
      _prevChildX = _prevChild == null
          ? (inverted ? spanWidth - padding + spacing : padding - spacing)
          : _prevChild.x;
      ...
      if (inverted) {
        ...
        //row space is full
        if ((spanWidth > 0 && reflow && _childXNew < padding) ||
      ...
      //calculate child X in normal mode
        ...
        if ((spanWidth > 0 &&
                reflow &&
                _childXNew + _childWidth + spacing > spanWidth) ||
            flowOrientation == FlowOrientation.VERTICAL) {
          _childXNew = padding;
          if (_prevChild != null) _newLine = true;
        }
      ...
      switch (alignV) {
        ...
        case AlignV.BOTTOM:
          _childYNew =
              (spanHeight - _totalHeight - _childHeight - padding).round();
          break;
      }
    ...
    num _spacer =
        (flowOrientation == FlowOrientation.HORIZONTAL && reflow == true)
            ? spacing
            : 0;
    span(spanWidth, _childYNew + _spacer + _childHeight + padding,
        refresh: false);
  }
