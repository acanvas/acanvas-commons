...
class Scrollbar extends Slider {
  ...
  void _pageScroll() {
    if (_snapToPage) {
      ...
      if ((_ori ? thumb.x : thumb.y) > (_ori ? mouseX : mouseY)) pageUp(); else if ((_ori ? thumb.x : thumb.y) + _thumbSize < (_ori ? mouseX : mouseY)) pageDown();
    }
  }
  ...
  void set pages(num pages) {
    if (pages != _pages) {
      _pages = max(pages, 1);
      _pageStepper.maximum = _pages - 1;
      _pageScrollDistance = _max / (_pages < 2 ? 1 : _pages - 1);
      _thumbSize = max(10, (_size / _pages).round());
      _ori ? thumb.width = _thumbSize : thumb.height = _thumbSize;
//				render();
    }
  }
  ...
  void redraw() {
    ...
    if (pos < 0) {
      ...
      offset = 0;
      num t = (_size / _pages).round() - offset * 4;
      thumbSize = max(10, t);
    }
