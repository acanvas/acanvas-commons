...
class Slider extends SpriteComponent {
  ...
  Slider(String ori, num min, num max, num size, [bool continuous = false]) : super() {
    _orientation = ori;
    _min = min;
    _max = max;
    _size = size;
    ...
    if (background == null) background = new Sprite();
    if (thumb == null) thumb = new Sprite();
    _thumbSize = _ori ? thumb.width : thumb.height;
  }
  ...
  void _updateThumbPosition() {
    num bgp = (_ori ? background.x : background.y);
    num pos = 0;
    if(_max != 0){
      pos = ((_value - _min) * ((_size - _thumbSize) / _max) + bgp).round();
    }
    
    _ori ? thumb.x = pos : thumb.y = pos;
  }
