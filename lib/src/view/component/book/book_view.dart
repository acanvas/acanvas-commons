	...
	// OVERRIDES:
		...
		@override 
		  void redraw() {
			...
			this.distortion = new DistortImage(widthAsSet/2, heightAsSet);
		}
	...
	// PAGEFLIP PROCESSING:
		 ...
		 */
		  void startPageFlip([MouseEvent event=null, bool attemptHover=false]) {
			...
			if (this.autoFlipActive && !this.tearActive && this.pageCorner.x >= 0 && this.pageCorner.x <= widthAsSet/2 && event != null){
			...
			this.renderShape.x = this.lastFlippedSide * widthAsSet/2;
			...
			this.pageCorner = new Point(this.lastFlippedCorner.x*widthAsSet/2, this.lastFlippedCorner.y*heightAsSet);
...
//				this.addSmoothGradients(ocf);
		 ...
		 */
		  void endPageFlip([MouseEvent event=null]) {
			...
			if (!this.tearActive){
				num x;
				num y = this.lastFlippedCorner.y*heightAsSet;
				if (this.lastFlippedSide == Page.LEFT){ // if left-hand page was flipped
					this.lastFlipSucceeded = (this.pageCornerTarget.x > widthAsSet/2);
					x = (this.lastFlipSucceeded) ? widthAsSet : 0;
				}else{ // if right-hand page was flipped
					this.lastFlipSucceeded = (this.pageCornerTarget.x < 0);
					x = (this.lastFlipSucceeded) ? -widthAsSet/2 : widthAsSet/2;
				}
		 ...
		 */
		  void drawHardPage(BitmapData front,BitmapData back) {
			...
			if (this.lastFlippedDirection == 1){
				w = this.pageCorner.x - (1-this.lastFlippedSide) * widthAsSet/2;
			}else{
				w = this.pageCorner.x - widthAsSet/2;
			}
			if (w < -widthAsSet/2) w = -widthAsSet/2;
			if (w >  widthAsSet/2) w =  widthAsSet/2;
			
			// calculate perspective:
			num closeness = sin(acos(w / (widthAsSet/2)));
			
			// define positions of page-corners:
			List pPoints = [];
			pPoints.add( new Point((1-this.lastFlippedSide)*widthAsSet/2, 0));
			pPoints.add( new Point(pPoints[0].x+w, 0-closeness*this.hardPerspective));
			pPoints.add( new Point(pPoints[0].x+w, heightAsSet+closeness*this.hardPerspective));
			pPoints.add( new Point((1-this.lastFlippedSide)*widthAsSet/2, heightAsSet));
			...
			if (this.lastFlippedSide == 0){
				bmd = (this.pageCorner.x > widthAsSet/2) ? back : front;
			}else{
		 ...
		 */
		  void evaluateTear(Point point) {
			...
			// evaluate:
			if ((point.x).round() == (1-this.lastFlippedCorner.x) * widthAsSet/2 && (point.y).round() == this.lastFlippedCorner.y * heightAsSet){
	...
	// GRADIENTS:
		 ...
		 */
		  void addHardGradients(List area) {
			...
			String tint;
			if (this.lastFlippedSide == Page.LEFT){ // if left-hand page was flipped
				tint = (this.pageCornerTarget.x > widthAsSet/2) ? Gradients.DARK : Gradients.LIGHT;
			}else{ // if right-hand page was flipped
	...
	// PAGEFLIP ASSISTANCE:
		 ...
		 */
		  bool movePageCornerTarget() {
			...
			if (!this.autoFlipActive){
				...
				x = this.mouseX;
				y = (!this.sideFlipActive) ? this.mouseY : this.lastFlippedCorner.y * heightAsSet;
				...
				if (this.sideFlipActive){
					...
					num xMin = 0 - this.lastFlippedSide * (widthAsSet/2);
					num xMax = xMin + widthAsSet;
					if (x < xMin) x = xMin;
					if (x > xMax) x = xMax;
			...
			if (this.autoFlipActive){
				// calculate coordinates:
				x = (!this.tearActive) ? this.pageCornerTarget.x : this.lastFlippedSide * (widthAsSet/2);
				y = this.pageCornerTarget.y;
				Point opposite = new Point(0,0);
				// if this is a normal auto-flip:
				if (!this.tearActive){
					opposite.x = this.lastFlippedSide * (widthAsSet/2) - widthAsSet*this.lastFlippedDirection;
				...
				if (this.tearActive){
					...
					opposite.y = (heightAsSet/2) + (heightAsSet*1.5 * directionY);
		 ...
		 */
		  void movePageCorner() {
			...
			if ((this._status == BookEvent.PAGEFLIP_ENDING || this._status == BookEvent.HOVER_ENDING) && !this.tearActive){
				...
				if (this.pageCorner.y > heightAsSet)	this.pageCorner.y = heightAsSet;
			}
		 ...
		 */
		  Point getCurrentCorner() {
			...
			// determine corner:
			corner.x = (this.mouseX < widthAsSet/2 ) ? 0 : 1;
			corner.y = (this.mouseY < heightAsSet/2) ? 0 : 1;
			// return value:
			return corner;
		}
		 ...
		 */
		  int getCurrentSide() {
			int side;
			if (!this.autoFlipActive){
				side = (this.mouseX <= widthAsSet/2) ? Page.LEFT : Page.RIGHT;
			}else{
				if (!this.tearActive){
		 ...
		 */
		  num getPageCornerYFromX(num x,[num curve=BookView.AUTO_FLIP_CURVE]) {
			num middleX	= this.lastFlippedSide * (widthAsSet/2) - (widthAsSet/2)*this.lastFlippedDirection;
 			num progress	= (middleX-x) / (widthAsSet/2).abs();
			num y;
			if (this.lastFlippedCorner.y == 0){
				y = heightAsSet * curve * (1-progress);
			}else{
				y = heightAsSet * (1-curve) + progress * (heightAsSet*curve);
			}
			return y;
		}
	...
	// ACCESSORS:
		 ...
		 */
		  void createRegions() {
			...
			this.regions[0]["TR"].x += widthAsSet / 2 - this.regionSize;
			this.regions[0]["BR"].x += widthAsSet / 2 - this.regionSize;
			this.regions[0]["BR"].y += heightAsSet - this.regionSize;
			this.regions[0]["BL"].y += heightAsSet - this.regionSize;
			// specify regions for right-hand page:
			this.regions.add(new Map());
			this.regions[1]["TL"] = new Rectangle(widthAsSet/2, 0, this._regionSize, this._regionSize);
			this.regions[1]["TR"] = new Rectangle(widthAsSet/2, 0, this._regionSize, this._regionSize);
			this.regions[1]["BR"] = new Rectangle(widthAsSet/2, 0, this._regionSize, this._regionSize);
			this.regions[1]["BL"] = new Rectangle(widthAsSet/2, 0, this._regionSize, this._regionSize);
			this.regions[1]["TR"].x += widthAsSet / 2 - this.regionSize;
			this.regions[1]["BR"].x += widthAsSet / 2 - this.regionSize;
			this.regions[1]["BR"].y += heightAsSet - this.regionSize;
			this.regions[1]["BL"].y += heightAsSet - this.regionSize;
			// specify side-flip regions for both pages:
			this.regions[0]["side"] = new Rectangle   (0,
													(heightAsSet-this._regionSize)/2,
													this._regionSize/2,
													this._regionSize);
			this.regions[1]["side"] = this.regions[0]["side"].clone();
			this.regions[0]["side"].x = widthAsSet-this._regionSize/2;
		}
