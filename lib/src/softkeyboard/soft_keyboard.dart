...
/**
	 ...
	 * 
	 */
class SoftKeyboard extends ManagedSpriteComponent {
  ...
  /** 
		 ...
		 */
  List<ManagedSpriteComponent> _configurations = new List<ManagedSpriteComponent>();
  ...
  SoftKeyboard(List<Layout> layouts, [num width = 320, num height = 160]) {
    ...
    if (width > 0) this.widthAsSet = width;
    if (height > 0) this.heightAsSet = height;
...
//				throw(error);
    ...
    _callout.setSize(key.height, (key.width * key.variants.length) + (key.variants.length - 1) * 5);
  ...
  Sprite _buildConfiguration(Layout layout) {
    ...
    ManagedSpriteComponent keyboard = new ManagedSpriteComponent();
  ...
  void redraw() {
    ...
    num hUnit = (widthAsSet - 2 * padding) / widest.relativeWidth;
    num vUnit = (heightAsSet - 2 * padding) / (rowCount + ((rowCount - 1) * currentLayout.verticalGap));
    ...
    for (int i = 0; i < rowCount; i++) {
      ...
      for (int j = 0; j < row.keys.length; j++) {
        ...
        key.setSize(key.relativeWidth * hUnit,  key.relativeHeight * vUnit);

        key.x = xPos;
        xPos += key.widthAsSet + currentLayout.horizontalGap * hUnit;
